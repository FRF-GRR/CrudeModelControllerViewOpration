...
namespace CRUDWITHModelViewController.Controllers
{
    public class CollageController : Controller
    {
        ...
        public void InsertUpdateData(CollageClass obj)
        {
            if (obj.Id == 0)
            {
                con.Open();
        ...
        public void DeleteData(CollageClass obj)
        {
            con.Open();
            SqlCommand cmd = new SqlCommand("delete from tblmanager where id = @id", con);
        ...
        public JsonResult EditData(CollageClass obj)
        {
            con.Open();
            SqlCommand cmd = new SqlCommand("select * from tblmanager where id=@id", con);
            cmd.Parameters.AddWithValue("@id", obj.Id);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            sda.Fill(dt);
